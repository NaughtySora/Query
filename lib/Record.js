...
class Record {
  ...
  #including(data) {
    const output = {};
    const { rename, transform } = this.#mapping;
  ...
  #delving(output, data) {
    ...
    for (const record of deep) {
      ...
      if (!name) throw new Error("Record object has to have name associated with object key");
  ...
  add(fields) {
    const create = this.#create;
    for (const option of fields) create.push(option);
    return this;
  }
