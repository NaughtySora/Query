...
describe("Query: units", () => {
  ...
  it("add", () => {
    ...
    new Query().add({ c: () => 42 }).process(mocks).forEach(item => {
      ...
      assert.equal(item.b, undefined);
      assert.equal(item.c, 42);
    });
  });
  ...
  it("compose", () => {
    ...
    res.forEach(item => {
      ...
      assert.equal(item.votes, undefined);
